    ...
    protected void doThing(MessageCreateEvent event) throws Exception {
        ...
        if(list.length >= 2) {
            ...
            if(forms.isEmpty()) {
                ...
                if(res != null) {
                    disableTimer();
                    event.getMember().ifPresent(member -> StaticStore.formAnimHolder.put(member.getId().asString(), new FormAnimHolder(forms, event.getMessage(), res, ch.getId().asString(), mode, frame, false, ((param & PARAM_DEBUG) > 0), lang, true, true)));
                }
            }
        } else {
            ch.createMessage(LangID.getStringByID("fimg_more", lang)).subscribe();
