...
public class ManualFormIconCache extends ConstraintCommand {
    public ManualFormIconCache(ROLE role, CommonStatic.Lang.Locale lang, IDHolder holder) {
    ...
    public void doSomething(@NotNull CommandLoader loader) throws Exception {
        ...
        if(search.isBlank()) {
            replyToMessageSafely(loader.getChannel(), LangID.getStringByID("formImage.fail.noParameter", lang), loader.getMessage(), a -> a);
            return;
        }
        ...
        if (forms.size() != 1) {
            replyToMessageSafely(loader.getChannel(), "Please be more accurate for unit name", loader.getMessage(), a -> a);
        ...
        if (!StaticStore.assetManager.uploadFormIconManual(f, id, file)) {
            replyToMessageSafely(loader.getChannel(), "Failed to upload unit icon file as cache...", loader.getMessage(), a -> a);
        } else {
            replyToMessageSafely(loader.getChannel(), "Successfully uploaded unit icon file as cache!", loader.getMessage(), a -> a);
        }
