...
public class AddStatusChannel extends ConstraintCommand {
    public AddStatusChannel(ROLE role, int lang, @Nullable IDHolder id) {
    ...
    public void doSomething(@NotNull CommandLoader loader) {
        ...
        for(int i = 0; i < filteredID.length; i++) {
            try {
                ...
                } else if(!((MessageChannel) channel).canTalk()) {
                    result.append(String.format(LangID.getStringByID("statuschan_addcant", lang), filteredID[i])).append("\n");
                } else if(holder.status.contains(filtered)) {
                    result.append(String.format(LangID.getStringByID("statuschan_addalready", lang), filteredID[i], filtered)).append("\n");
                } else {
