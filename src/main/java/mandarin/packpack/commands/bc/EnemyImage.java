    ...
    public void doSomething(MessageCreateEvent event) throws Exception {
        ...
        if(list.length >= 2) {
            ...
            if(enemies.isEmpty()) {
                ...
                if(res != null) {
                    event.getMember().ifPresent(member -> StaticStore.enemyAnimHolder.put(member.getId().asString(), new EnemyAnimHolder(enemies, res, ch.getId().asString(), mode, frame, ((param & PARAM_TRANSPARENT) > 0), ((param & PARAM_DEBUG) > 0), lang, false, false)));
                }
            }
        } else {
            ch.createMessage(LangID.getStringByID("eimg_more", lang)).subscribe();
