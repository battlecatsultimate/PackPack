...
public class TrueFormAnalyzerHolder extends FileAnalyzerHolder {
    ...
    public TrueFormAnalyzerHolder(Message msg, Message author, int uID, boolean isSecond, List<CellData> cellData, List<AbilityData> procData, List<FlagCellData> abilityData, List<FlagCellData> traitData, String channelID, File container, int lv, String[] name, int lang, List<String> requiredFiles) {
    ...
    public void perform(Map<String, File> fileMap) throws Exception {
        CustomMaskUnit[] units = new CustomMaskUnit[1];
        ...
        File maanim = fileMap.get(Data.trio(uID)+"_s02.maanim");
        ...
        units[0] = new CustomMaskUnit(statReader.readLine().split(","), curve, ma, rare);

        statReader.close();

        EntityHandler.generateStatImage(msg.getChannel(), cellData, procData, abilityData, traitData, units, name, container, container, lv, !isSecond, null, trueForm, true, uID, lang);
    }
