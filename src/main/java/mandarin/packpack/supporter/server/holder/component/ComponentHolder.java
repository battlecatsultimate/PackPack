...
public abstract class ComponentHolder extends Holder {
    ...
    public ComponentHolder(@Nullable Message author, long userID, long channelID, @Nonnull Message message, @Nonnull CommonStatic.Lang.Locale lang) {
    ...
    private boolean canHandleEvent(GenericComponentInteractionCreateEvent event) {
        return event.getChannel().getIdLong() == channelID
                && event.getMessage().getIdLong() == message.getIdLong()
