...
class CardPackCostHolder(author: Message, channelID: String, private val message: Message, private val pack: CardPack) : ComponentHolder(author, channelID, message.id) {
    override fun clean() {
    ...
    override fun onEvent(event: GenericComponentInteractionCreateEvent) {
        when(event.componentId) {
            "cf" -> {
                ...
                connectTo(CatFoodCostHolder(authorMessage, channelID, message, pack))
            }
            "shard" -> {
                val input = TextInput.create("amount", "Amount", TextInputStyle.SHORT)
                    .setRequired(true)
                ...
                connectTo(PlatinumShardCostHolder(authorMessage, channelID, message, pack))
            }
            "add" -> {
