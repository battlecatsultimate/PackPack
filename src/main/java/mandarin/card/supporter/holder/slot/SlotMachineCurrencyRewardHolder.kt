...
class SlotMachineCurrencyRewardHolder(
    ...
    private val actualEmojis = SlotEmojiContainer.loadedEmoji.filter { e -> emojiName in e.name && !slotMachine.content.filter { c -> c !== content }.any { c -> c.emoji?.name == e.name && c.emoji?.id == e.id } }.toMutableList()
