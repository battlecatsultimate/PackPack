...
class UnregisterEmojiServer : Command(LangID.EN, true) {
    override fun doSomething(loader: CommandLoader) {
        ...
        if (g.idLong !in SlotEmojiContainer.registeredServer) {
            replyToMessageSafely(loader.channel, "This guild isn't registered as emoji guild yet!", loader.message) { a -> a }

            return
        }

        SlotEmojiContainer.unregisterServer(g)

        replyToMessageSafely(loader.channel, "Successfully unregistered the guild\n\nGuild Name : ${g.name}\nID : ${g.idLong}\nNumber of Emojis : ${g.emojis.size}", loader.message) { a -> a }
    }
}