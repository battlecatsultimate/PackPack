...
class RemoveECC : Command(CommonStatic.Lang.Locale.EN, true) {
    override fun doSomething(loader: CommandLoader) {
        ...
        if (contents.size < 2) {
            replyToMessageSafely(loader.channel, "Format : `cd.recc [User ID/Mention]`", loader.message) { a -> a }
        ...
        replyToMessageSafely(loader.channel, "Are you sure you want to remove ECC status from this user? This cannot be undone!", loader.message, { a -> registerConfirmButtons(a, lang)}) { msg ->
            StaticStore.putHolder(loader.user.id, ConfirmPopUpHolder(loader.message, loader.user.id, loader.channel.id, msg, { e ->
                ...
                TransactionLogger.logECCCancel(id, loader.user.idLong, inventory)

                inventory.cancelECC(loader.guild, id)

                CardBot.saveCardData()

                e.deferEdit()
                    .setContent("Successfully removed ECC from suer <@$id>")
                    .setComponents()
                ...
                e.deferEdit()
                    .setContent("ECC removal canceled")
                    .setComponents()
                    .setAllowedMentions(arrayListOf())
                    .mentionRepliedUser(false)
                    .queue()
