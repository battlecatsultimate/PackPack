...
class Trade : Command(LangID.EN, true) {
    override fun doSomething(event: GenericMessageEvent) {
        ...
        if (!CardData.isMod(m) && m.id != StaticStore.MANDARIN_SMELL) {
            replyToMessageSafely(ch, "This command can be run by only moderators. If you want to trade with others, " +
                    "request moderators to open trading session for you and other trader", getMessage(event)) { a -> a }
